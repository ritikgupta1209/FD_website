            ...
            <div class="row">
                    ...
                    <div class="sidebar-item">
                        <div class="make-me-sticky">
                            <div class="d-none d-lg-block">
                                ...
                                <?php include('include/components/profilePageLeftDiv.php'); ?>
                ...
                <div class="col-lg-9 col-md-12">
                    <!-- <div class="heading mb-4">
                        <h2 class="fw-bold">All Post<span>.</span></h2>
                    </div> -->
                    <div class="post-profile-post">
                        <input type="hidden" id="usernameId" value="<?php echo $username_req; ?>">
                        <?php
                        $count_query_profile_post = "SELECT count(*) as allcount FROM `stories` INNER JOIN `user_login` ON `stories`.`user_uid` = `user_login`.`user_uid` WHERE `post_status` = 'published' AND `unlisted` = 'false' AND `username`='$username_req'";
                        ...
                        $query = "SELECT `stories`.*,`user_login`.`username`, `user_login`.`name`, `user_login`.`profile` FROM `stories` INNER JOIN `user_login` ON `stories`.`user_uid` = `user_login`.`user_uid` WHERE `post_status` = 'published' AND `unlisted` = 'false' AND `username`='$username_req' ORDER BY post_id DESC LIMIT 0," . $limitProfilePost;
                                        ...
                                        <div class="card-body px-2">
                                            ...
                                            <div class="d-flex flex-wrap gap-2 mb-3">
                                                ...
                                                foreach ($tag_name as $key => $val) {
                                                    echo '<a href="' . $val . '" class="topic fw-bold py-0 px-3">' . $val . '</a>';
                                                }
                                                ?>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#loadBtnProfilePost', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'php/loadMoreProfilePost.php',
                    data: {
                        'row': row,
                        'username': username
                    },
