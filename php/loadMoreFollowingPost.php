...
if (isset($_POST['row'])) {
    ...
    $query = "SELECT `stories`.*, `user_login`.`username`,`user_login`.`name`, `user_login`.`profile`FROM `follow` LEFT JOIN `stories`ON (`stories`.`user_uid`='$user_uid' 
                                    OR `stories`.`user_uid` = `follow`.`followed_user_uid`) LEFT JOIN `user_login` ON `stories`.`user_uid`=`user_login`.`user_uid`WHERE (`follow`.`following_user_uid`='$user_uid' OR `follow`.`following_user_uid`=NULL) AND(`stories`.`post_status` = 'published' AND `stories`.`unlisted` = 'false') ORDER BY post_id DESC LIMIT  " .$start . "," . $limit;
