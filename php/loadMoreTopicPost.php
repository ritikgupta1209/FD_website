...
if (isset($_POST['row'])) {
    ...
    $limit = 2;
    $query = "SELECT `stories`.*,`user_login`.`username`, `user_login`.`name`, `user_login`.`profile` FROM `stories` INNER JOIN `user_login` ON `stories`.`user_uid` = `user_login`.`user_uid` WHERE `post_status` = 'published' AND `unlisted` = 'false' AND `stories`.`post_tags` LIKE '%$topic_req%' ORDER BY post_id DESC LIMIT " . $start . "," . $limit;
