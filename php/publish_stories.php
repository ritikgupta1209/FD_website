...
if (isset($_POST['story_title'])) {
    ...
    if ($_POST['post_id'] == "") {
       ...
       `post_tags`,`unlisted`,`pin_story`, `post_status`,`post_slug`,`meta_title`,`meta_description`,
       `from_ip`,`from_browser`,`created_at`) VALUE('$post_uid','$user_uid','$title', '$content','$image',
       '$tags','$unlisted','$pin_story', 'published',	'$post_slug','$meta_title','$meta_description',
        '$from_ip', '$from_browser', '$date_now')";
       ...
       // echo $link->insert_id;
        ...
        $link->query("UPDATE `stories` SET `post_title` = '$title', `post_content` = '$content',`featured_image` = '$image', `post_tags`='$tags', `unlisted` = '$unlisted',`pin_story`='$pin_story', `post_slug`='$post_slug',`meta_title`='$meta_title',`meta_description`='$meta_description', `updated_at`='$date_now', `post_status` = 'published' WHERE `post_id` = '$post_id'");
    }
}
