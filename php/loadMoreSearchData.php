...
if (isset($_POST['row'])) {
    ...
    $dat = $_POST['dat'];
    $limit = 2;
    $query = "SELECT `stories`.*,`user_login`.`username`, `user_login`.`name`, `user_login`.`profile` FROM `stories` INNER JOIN `user_login` ON `stories`.`user_uid` = `user_login`.`user_uid`  WHERE `post_title` LIKE '%$dat%' OR `post_title` LIKE '%$dat%' ORDER BY post_id DESC LIMIT " . $start . "," . $limit;
