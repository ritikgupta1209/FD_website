                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="./">Dashboard</a></li>
                                <li class="breadcrumb-item"><a href="followusers">Followers</a></li>
                                <li class="breadcrumb-item active" aria-current="page">All followers</li>
                        </ol>
                    </nav>
                </div>
                                ...
                                <div class="card-body table-responsive">
                                    ...
                                    <table id="zero-conf" class="display" style="width:100%">
                                        ...
                                        <tbody>
                                            ...
                                            $result = mysqli_query($link, "SELECT * FROM `follow`");
                                           
                                            $i = 1;
                                            while ($row = mysqli_fetch_array($result)) {
                                                $id = $row['follow_id'];
                                                $followid=$row['following_user_uid'];
                                                $followingid=$row['followed_user_uid'];
                                            $result2 = mysqli_query($link, "SELECT * FROM `user_login` WHERE `user_uid` = '$followid' ");
                                            $row2 = mysqli_fetch_array($result2);
                                            $follower=$row2['name'];
                                            
                                            $result3 = mysqli_query($link, "SELECT * FROM `user_login` WHERE `user_uid` = '$followingid' ");
                                            $row3 = mysqli_fetch_array($result3);
                                            $following=$row3['name'];

                                                echo "<input type=\"hidden\" id=\"follow_id\" value=\"{$row['follow_id']}\">";
                                                       
                                                   echo "<tr><td>".$i."</td><td><a href= \"viewuser.php?id={$row2['id']}\" >$follower</a></td><td><a href= \"viewuser.php?id={$row3['id']}\" data-toggle= \"tooltip \" data-original-title= \"View \" color=\"inherit\" text-decoration:\"none\">$following</a></td>
                                                   </tr>";
